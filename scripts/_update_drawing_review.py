# -*- coding: utf-8 -*-
"""Rewrite drawing-review/page.tsx with 2D/3D comparison feature integrated."""
...
export default function DrawingReviewPage() {
    ...
    const handleGeneratePPT = async () => {
        ...
        try {
            const res = await fetch("/api/dr-ppt", {
                ...
                body: JSON.stringify({ projectInfo, issues }),
            })
    ...
    const reportTab = (
        <div className="space-y-6">
            <Row gutter={16}>
                <Col span={16}>
                    <Card title={<span className="flex items-center gap-2"><FilePdfOutlined className="text-blue-600" />DR 报告</span>} className="min-h-[400px]">
                        {generated ? (
                            <div className="space-y-4">
                                <Alert message={`DR 报告已生成 · 共 ${issues.length + 3} 页`} type="success" showIcon />
                                <div className="grid grid-cols-4 gap-3">
                                    {(() => {
                                        const allPages = [
                                            { title: "封面页", img: "/api/serve-file/ppt-slides/slide_1.png" },
                                            { title: "问题汇总表", img: "/api/serve-file/ppt-slides/slide_2.png" },
                                            ...issues.map((_, i) => ({ title: `#${i+1} 问题详情`, img: `/api/serve-file/ppt-slides/slide_${i+3}.png` })),
                                            { title: "评审结论", img: `/api/serve-file/ppt-slides/slide_${issues.length+3}.png` },
                                        ]
                                        return allPages.map((page, i) => (
                                            <div key={i} className="border rounded-lg p-2 bg-slate-50 hover:border-blue-400 hover:shadow-md transition-all cursor-pointer group"
                                                onClick={() => setPreviewSlide({ visible: true, index: i, pages: allPages })}>
                                                <div className="aspect-[4/3] bg-white rounded border border-slate-200 overflow-hidden mb-1 relative">
                                                    <img src={page.img} alt={page.title} className="w-full h-full object-contain group-hover:scale-105 transition-transform" />
                                                    <div className="absolute top-1 right-1 bg-black/60 text-white text-[9px] px-1.5 py-0.5 rounded font-mono">P{i + 1}</div>
                                                </div>
                            ...
                            <div className="flex flex-col items-center justify-center py-16">
                                {generating ? (
                                    <><LoadingOutlined style={{ fontSize: 48 }} className="text-blue-600 mb-4" /><p className="font-bold text-lg mb-2">正在生成 DR 报告...</p><Progress percent={75} className="max-w-xs" status="active" /></>
                                ) : (
                                    <><FilePdfOutlined style={{ fontSize: 64 }} className="text-gray-200 mb-4" /><p className="text-gray-500 mb-4">根据 {issues.length} 个评审问题自动生成 PPT 报告</p><Button type="primary" size="large" icon={<ThunderboltOutlined />} onClick={handleGeneratePPT}>一键生成 DR 报告</Button></>
                                )}
                            </div>
                        )}
                    </Card>
                ...
                <Col span={8}>
                    <Card title="报告概要" size="small" className="mb-4">
                        <div className="space-y-2">
                            ...
                            <div className="flex justify-between text-sm"><span className="text-gray-500">问题总数</span><span className="font-bold text-lg">{stats.total}</span></div>
                            <div className="flex justify-between text-sm"><span className="text-red-600">严重</span><Badge count={stats.critical} style={{ backgroundColor: "#dc2626" }} /></div>
                            <div className="flex justify-between text-sm"><span className="text-orange-500">重要</span><Badge count={stats.major} style={{ backgroundColor: "#f59e0b" }} /></div>
                            <div className="flex justify-between text-sm"><span className="text-green-600">一般</span><Badge count={stats.minor} style={{ backgroundColor: "#16a34a" }} /></div>
                            {comparisonResult && comparisonResult.summary.mismatched > 0 && (
                                <>
                                    <Divider className="my-2" />
                                    <div className="flex justify-between text-sm"><span className="text-blue-600">2D/3D 不匹配</span><Badge count={comparisonResult.summary.mismatched} style={{ backgroundColor: "#2563eb" }} /></div>
                                </>
                            )}
                        </div>
                    </Card>
                    <Card title="PPT 结构" size="small">
                        <Timeline items={[
                            { color: "blue", children: <span className="text-xs">封面页</span> },
                            { color: "blue", children: <span className="text-xs">问题汇总表</span> },
                            { color: "red", children: <span className="text-xs">问题详情 × {issues.length} 页</span> },
                            { color: "green", children: <span className="text-xs">评审结论 & 签字</span> },
                        ]} />
                    </Card>
                </Col>
            </Row>
...
with open("/Users/wangpeng/Downloads/cisheng/src/app/drawing-review/page.tsx", "w", encoding="utf-8") as f:
    f.write(content)
print("OK: drawing-review/page.tsx written")
