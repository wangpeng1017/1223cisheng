...
async function main() {
  ...
  // Products
  await pool.query(`INSERT IGNORE INTO products (product_id, name, category, spec, status) VALUES
  ...
  // Knowledge articles
  await pool.query(`INSERT IGNORE INTO knowledge_articles (id, title, tags, views, content) VALUES
  ...
  // Defects
  await pool.query(`INSERT IGNORE INTO defects (id, code, name, category, severity, description, solution, occurrence) VALUES
  ...
  // Q-Plan items
  await pool.query(`INSERT IGNORE INTO qplan_items (id, plan_type, category, item, spec_limit, tool, sample_level) VALUES
  ...
  // Reliability tests
  await pool.query(`INSERT IGNORE INTO reliability_tests (id, title, description, status, planned_date) VALUES
  ...
  // Process steps
  await pool.query(`INSERT IGNORE INTO process_steps (id, route_name, step_no, name, params, status, sort_order) VALUES
  ...
  // PFMEA risks
  await pool.query(`INSERT IGNORE INTO pfmea_risks (id, operation, failure_mode, effect, measure, sod, status) VALUES
  ...
  // Prototype batches
  await pool.query(`INSERT IGNORE INTO prototype_batches (id, batch_no, quantity, result, batch_date) VALUES
  ...
  // NPI stage stats (for dashboard chart)
  await pool.query(`INSERT IGNORE INTO npi_stage_stats (id, stage, actual_rate, target_rate) VALUES
    (1, 'DROP', 85, 90),
    (2, 'Proto', 72, 80),
    (3, 'EVT', 65, 75),
    (4, 'DVT', 92, 95),
  ...
  // AI review tasks
  await pool.query(`INSERT IGNORE INTO ai_review_tasks (task_id, drawing_name, file_type, status, duration_seconds, total_issues, critical_count, warning_count, info_count, conclusion, created_at) VALUES
  ...
  // AI review issues
  await pool.query(`INSERT IGNORE INTO ai_review_issues (id, task_id, severity, category, title, description, suggestion) VALUES
  ...
  // System users
  await pool.query(`INSERT IGNORE INTO system_users (id, name, email, role, title, avatar, join_date, total_reviews, active_projects) VALUES
    (1, '张工', 'zhang.g@cisheng-tech.com', 'admin', '高级研发工程师 / NPI 组长', '', '2022-05-18', 142, 12)
  `)

  // User activities
  await pool.query(`INSERT IGNORE INTO user_activities (id, user_id, content, created_at) VALUES
  ...
  // Notifications
  await pool.query(`INSERT IGNORE INTO notifications (id, user_id, type, content, status, color) VALUES
  ...
  // Roles
  await pool.query(`INSERT IGNORE INTO roles (id, name, member_count, description) VALUES
  ...
  // Audit logs
  await pool.query(`INSERT IGNORE INTO audit_logs (id, user_name, module, action, ip, status, created_at) VALUES
  ...
  // System settings
  await pool.query(`INSERT IGNORE INTO system_settings (id, setting_key, setting_name, description, enabled, category) VALUES
  ...
  // Watched projects
  await pool.query(`INSERT IGNORE INTO watched_projects (id, user_id, project_name) VALUES
    (1, 1, '新款磁体单元 Gen3'),
    (2, 1, 'Alpha-S1 振膜模组'),
    (3, 1, '车规级磁路平台')
  `)
  ...
  // DR reviews
  await pool.query(`INSERT IGNORE INTO dr_reviews (id, project_name, product_name, part_number, customer, reviewer, review_date, drawing_version, status) VALUES
  ...
  // DR issues
  await pool.query(`INSERT IGNORE INTO dr_issues (id, review_id, seq_no, ppt_page, fai_spc, category, description, drawing_spec, suggestion, pd_opinion, severity) VALUES
  ...
  // Also seed some projects if table is empty
  const [projectRows] = await pool.query<mysql.RowDataPacket[]>("SELECT COUNT(*) as cnt FROM projects")
  if (projectRows[0].cnt === 0) {
    await pool.query(`INSERT INTO projects (project_no, name, product_name, product_code, manager, creator, deadline, status, progress, risk) VALUES
    ...
    `)
    // Add some tasks for projects
    const [pRows] = await pool.query<mysql.RowDataPacket[]>("SELECT id, project_no FROM projects")
    for (const p of pRows) {
      await pool.query(`INSERT INTO project_tasks (project_id, name, owner, progress, status) VALUES
        (?, '图纸评审', '张工', ?, ?),
        (?, 'DFM分析', '王工', ?, ?),
        (?, 'Q-Plan制定', '赵工', ?, ?)
      `, [
        p.id, p.id <= 3 ? 100 : 50, p.id <= 3 ? '已完成' : '进行中',
        p.id, p.id <= 2 ? 80 : 30, p.id <= 2 ? '进行中' : '未开始',
        p.id, p.id <= 1 ? 60 : 0, p.id <= 1 ? '进行中' : '未开始',
      ])
    }
    console.log("✓ Seeded projects + tasks")
  }
